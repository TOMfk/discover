...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function update() {
      ...
      this.updateBanTime(year, month);
      this.updateHoliday(year, month);
      this.updateMark(year, month);
      this.updateSelector(year,month);
      this.updateFestival(year, month);
      this.updateEvent(year, month);
...
//        this.selectDay = daysElement[i];
    ...
    //设置时间段时禁止掉不在范围内的
  	...
  	key:'updateBanTime',
  	value: function  updateBanTime(year,month) {
  		var litterTime = this._options.timeBanRange.startTime.split('-');
      var bigTime = this._options.timeBanRange.endTime.split('-');
     
      var banYear = [parseInt(litterTime[0]),parseInt(bigTime[0])];
      var banMonth = [parseInt(litterTime[1]),parseInt(bigTime[1])];
      var banDay = [parseInt(litterTime[2]),parseInt(bigTime[2])];
      
      if (this._options.showRange) {
      	 ...
      	 daysElement.forEach(function(v,i) {
      	 	...
      	 	if ( year < banYear[0] || year > banYear[0] ) {
      	 		
      	 		 if(v.getAttribute('class').indexOf('sc-othermenth') <0 ) v.classList.add('sc-othermenth');
      	 		 
      	 	} else if ( (year == banYear[0] && month < banMonth[0]) || (year == banYear[1] && month > banMonth[1]) ){
      	 		
      	 		if(v.getAttribute('class').indexOf('sc-othermenth') < 0 ) v.classList.add('sc-othermenth');
      	 		
      	 	} else if( ( year == banYear[0] && month == banMonth[0] && day <= banDay[0] ) ) {
      	 	 
      	 	  if(v.getAttribute('class').indexOf('sc-othermenth') < 0 ) v.classList.add('sc-othermenth');
      	 	  
      	 	} else if ( year == banYear[1] && month == banMonth[1] && (day > banDay[1])  ) {
      ...
      if (options.showMark) {
        ...
        if (data) {
          daysElement.forEach(function (v, i) {
            ...
            var day = v.querySelector('.day').innerHTML;//这里v前面不知为何有个+ 我去掉了
            if (day == 1) currentmonth++;

            if (data[year + '-' + currentmonth + '-' + day]) {
              v.classList.add('sc-mark');
              var markDiv = '<div class="markTxt">'+data[year + '-' + currentmonth + '-' + day]+'</div>';
              v.innerHTML = v.innerHTML + markDiv;
    ...
    key: 'updateSelector',
    value: function updateSelector(year, month) {
      var options = this._options;
      ...
      if (options.showMore) {
        ...
        //取得节日数据
        var data = options.selectDays;
        if (data) {
          daysElement.forEach(function (v, i) {

            var day = v.querySelector('.day').innerHTML;
            if (day == 1) currentmonth++;
            var str = year + '-' + currentmonth + '-' + day ;
    ...
    key: 'updateEvent',
    value: function updateEvent(year, month) {
      var daysElement = this.arrayfrom(this.container.querySelectorAll('.sc-item'));
      var options = this._options;
      ...
      daysElement.forEach(function (v, i) {
        ...
        v.onclick = function () {
        	if(this.getAttribute('class').indexOf('sc-othermenth')>0) return false;
          calendar.selectDay = v;
          var selectDay = year+'-'+month+'-'+v.querySelector('.day').innerHTML;
          
          var pre = container.querySelector('.sc-selected');
          var len = this.getAttribute('class').indexOf('sc-selected');
          
	          if (len > 0 && selectDays.length > 0) {
	          	this.classList.remove('sc-selected');
	          	for(var m=0;m<selectDays.length;m++){
	          		if(selectDays[m] == selectDay){
	          			selectDays.splice(m,1);
	          		}
	          	}
	          }else {
		          	if(selectDays.length < maxLength) {
		          		this.classList.add('sc-selected');
		          		selectDays.push(selectDay);
		          	}
          }
	          //console.log(calendar.getSelectedDay());
    ...
    key: 'addMark',
    value: function addMark(markInfo) {
    	for(var key in markInfo){
        this._options.mark[key] = markInfo[key];
    	}
      this.update();
    }
...
//    var selectDay = this.selectDay.querySelector('.day').innerHTML;
			var selectTimes = this._options.selectDays;
      return selectTimes;
    }
